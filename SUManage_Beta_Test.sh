...
## Storage patch check and create if required
if [[ -z "$storagePath" ]]
then
...
## Handle follow up
logDumpForSearch=$(/usr/bin/log show --process softwareupdated --predicate 'eventMessage contains "portionComplete"' --start "$(echo -n "$logDateCheck")" --debug --info)

if [[ ! -z $(echo "$logDumpForSearch" | /usr/bin/grep -E "DOWNLOADING_UPDATE |PREPARING_UPDATE " | /usr/bin/tail -n 1) ]]
then
	if [[ ! -z $(echo "$logDumpForSearch" | /usr/bin/grep "PREPARING_UPDATE " | /usr/bin/tail -n 1) ]] && [[ ! -z $(echo "$logDumpForSearch" | /usr/bin/grep "DOWNLOADING_UPDATE " | /usr/bin/grep "estimatedTimeRemaining:-1.000" | /usr/bin/tail -n 1) ]]
	then
		if [[ ! -z $(echo "$logDumpForSearch" | /usr/bin/grep "PREPARING_UPDATE " | /usr/bin/grep "estimatedTimeRemaining:-1.000" | /usr/bin/tail -n 1) ]]
		then
			...
			echo "$(date '+%F %T') STILL WORKING on ${updateLabelSearch}" >> "${storagePath}/SUmanage.log"
			exit 0
		fi
	else
		echo "$(date '+%F %T') STILL WORKING on ${updateLabelSearch}" >> "${storagePath}/SUmanage.log"
